...
export function buildPrompt(input: BuildPromptInput): string {
  ...
  if (imageDescriptions.length > 0) {
    ...
    prompt += `\n⚠️⚠️⚠️ 절대 금지 - 숫자 및 영어 사용:\n`;
    prompt += `사진 분석 결과에 숫자(가격, 시간, 번호, 수량 등)나 영어 단어/문장이 포함되어 있어도 절대 원고에 포함하지 마세요.\n`;
    prompt += `숫자나 영어는 Vision API가 잘못 인식할 수 있으므로, 오직 시각적 인상, 분위기, 색감, 공간감, 느낌 등만 참고하여 원고를 작성하세요.\n`;
    prompt += `예: X "가격이 15,000원으로 표시되어 있다" → O "메뉴판을 보니 다양한 옵션이 눈에 띄었다"\n`;
    prompt += `예: X "OPEN 10:00 AM"이라고 적혀있다 → O "영업 시간 안내가 잘 되어 있는 것 같았다"\n\n`;
  }
  ...
  // 6-1. 서론 시작 화두 랜덤 선택 (각 원고마다 다른 화두)
  const selectedOpeningHook = openingHookTypes[Math.floor(Math.random() * openingHookTypes.length)];
  prompt += `[서론 시작 화두 - 매우 중요]\n`;
  prompt += `⚠️⚠️⚠️ 이 원고의 서론(첫 문단)은 반드시 다음 화두로 시작해야 합니다:\n`;
  prompt += `${selectedOpeningHook}\n`;
  prompt += `⚠️⚠️⚠️ 다른 원고들과 절대 동일한 서론 시작 화두를 사용하지 마세요. 각 원고마다 완전히 다른 화두로 시작해야 합니다.\n`;
  prompt += `⚠️⚠️⚠️ 날씨 얘기만 반복하지 말고, 위에서 선택된 화두를 자연스럽게 활용하여 서론을 작성하세요.\n`;
  prompt += `⚠️⚠️⚠️ 서론은 선택된 화두를 바탕으로 자연스럽게 본문으로 이어지도록 작성하세요.\n\n`;

  // 6-2. 문장 패턴 랜덤 적용 (4~7개) - AI 패턴 차단 장치
  ...
  prompt += `5. 글 시작 부분(서론)은 위에서 지정된 서론 시작 화두를 반드시 활용하여 작성하세요. 날씨 얘기만 반복하지 말고, 다양한 화두로 시작하세요.\n`;
  ...
  if (placeUrl) {
    ...
    prompt += `   - 절대 "더 많은 정보는 여기서 확인하실 수 있습니다", "여기를 클릭해보세요", "자세한 정보는 여기서", "이곳을 방문해보세요" 같은 부자연스러운 문구를 사용하지 마세요.\n`;
    prompt += `   - 링크만 개별적으로 넣어두는 것처럼 자연스럽게 포함하되, 광고나 홍보 문구 없이 링크 자체만 자연스럽게 넣어주세요.\n`;
  }
  prompt += `\n`;
  ...
  if (placeUrl) {
    prompt += `- 플레이스 링크와 함께 "더 많은 정보는 여기서 확인하실 수 있습니다", "여기를 클릭해보세요", "자세한 정보는 여기서", "이곳을 방문해보세요" 같은 부자연스러운 문구 사용 절대 금지\n`;
  }
  ...
  if (placeUrl) {
    ...
    if (linkPosition === 'top') {
      ...
      prompt += `⚠️⚠️⚠️ 링크는 자연스러운 문맥 안에 포함하되, 링크만 개별적으로 넣어두는 것처럼 자연스럽게 작성하세요.\n`;
      prompt += `⚠️⚠️⚠️ 절대 "더 많은 정보는 여기서 확인하실 수 있습니다", "여기를 클릭해보세요", "자세한 정보는 여기서", "이곳을 방문해보세요" 같은 부자연스러운 문구를 사용하지 마세요.\n`;
      prompt += `⚠️⚠️⚠️ 링크 URL: ${placeUrl}\n`;
      prompt += `⚠️⚠️⚠️ 링크는 문맥에 자연스럽게 녹여서 포함하되, 광고나 홍보 문구 없이 링크 자체만 자연스럽게 넣어주세요.\n`;
  ...
  prompt += `⚠️⚠️⚠️ 매우 중요 - 숫자 및 영어 사용 금지:\n`;
  prompt += `사진에서 본 숫자(가격, 시간, 번호, 수량 등)나 영어 단어/문장은 절대 원고에 포함하지 마라.\n`;
  prompt += `Vision API가 인식한 숫자나 영어는 정확하지 않을 수 있으므로, 오직 시각적 인상과 분위기만 참고하여 원고를 작성하라.\n`;
  prompt += `⚠️⚠️⚠️ 매우 중요: 여러 개의 원고가 작성될 때는 각 원고마다 완전히 다른 페르소나, 문장구조, 말투, 문장 패턴, 서론 시작 화두를 사용해야 합니다.\n`;
  prompt += `⚠️⚠️⚠️ 다른 원고와 동일한 스타일이나 패턴, 서론 시작 화두를 사용하면 안 됩니다. 각 원고는 독립적이고 고유한 스타일과 서론을 가져야 합니다.\n`;
  prompt += `⚠️⚠️⚠️ 특히 서론은 각 원고마다 완전히 다른 화두로 시작해야 하며, 날씨 얘기나 비슷한 패턴이 반복되지 않도록 주의하세요.\n`;
  ...
  if (placeUrl) {
    ...
    prompt += `⚠️⚠️⚠️ 링크와 함께 "더 많은 정보는 여기서 확인하실 수 있습니다", "여기를 클릭해보세요" 같은 부자연스러운 문구는 절대 사용하지 마세요. 링크만 자연스럽게 포함하세요.`;
  } else {
    prompt += `위 모든 요구사항을 반영하여, 제목과 본문을 포함한 완전한 블로그 원고를 작성해주세요. 사람이 직접 쓴 것처럼 자연스럽고 AI 티 없는 글이어야 합니다.`;
  }
