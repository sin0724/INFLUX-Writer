...
export function buildPrompt(input: BuildPromptInput): string {
  ...
  if (imageDescriptions.length > 0) {
    ...
    prompt += `[Vision 활용 규칙 - 매우 중요]\n`;
    prompt += `🚫 절대 금지:\n`;
    prompt += `- 분석 결과 문장을 그대로 사용하는 것 금지\n`;
    prompt += `- 사진에서 본 숫자, 영어, 텍스트를 원고에 포함하는 것 절대 금지\n`;
    prompt += `- 시험지, 문제지, 교재, 학습 자료의 내용을 언급하는 것 절대 금지\n`;
    prompt += `- 구체적인 디테일을 나열하는 것 금지 (예: "책상이 3개 있다", "영어 단어가 보인다")\n\n`;
    prompt += `✅ 올바른 활용 방법:\n`;
    prompt += `- 시각적 인상과 분위기만 간접적으로 활용\n`;
    prompt += `- 느낌과 감정 중심으로 자연스럽게 표현\n`;
    prompt += `- 예: "밝고 깔끔한 공간에서 학습할 수 있어 편안했어요"\n`;
    prompt += `- 예: "정돈된 분위기에서 집중하기 좋았어요"\n`;
    prompt += `- 예: "깨끗하고 쾌적한 환경이 인상적이었어요"\n\n`;
    
    // 후기형일 때 추가 강조
    if (contentType === 'review') {
      prompt += `[후기형 작성 시 Vision 활용 - 특별 주의]\n`;
      prompt += `- 후기글은 실제 경험자의 느낌과 감정이 중심이어야 합니다\n`;
      prompt += `- 사진을 너무 디테일하게 묘사하지 말고, 전체적인 분위기와 느낌만 자연스럽게 표현하세요\n`;
      prompt += `- 숫자나 영어가 포함된 분석 결과는 완전히 무시하고, 오직 시각적 인상만 간접적으로 활용하세요\n`;
      prompt += `- 예: "공간이 깔끔해서 좋았어요" (O) / "책상이 5개 있고 영어 단어가 보인다" (X)\n\n`;
    }
    
    // 학원 카테고리일 때 특별 경고
    if (category === '학원/스터디카페') {
      prompt += `[학원/스터디카페 특별 주의사항]\n`;
      prompt += `⚠️ 학원 관련 사진에는 시험지, 문제지, 교재, 학습 자료가 포함될 수 있습니다\n`;
      prompt += `⚠️ 이런 사진에서 숫자나 영어가 보여도 절대 원고에 포함하지 마세요\n`;
      prompt += `⚠️ 오직 공간의 분위기, 학습 환경, 느낌만 자연스럽게 표현하세요\n`;
      prompt += `⚠️ 예: "학습하기 좋은 분위기" (O) / "수학 문제가 보인다" (X) / "영어 단어가 있다" (X)\n\n`;
    }
  }
  ...
  prompt += `🚫 사진 관련 절대 금지 (매우 중요):\n`;
  prompt += `   - 사진에서 본 모든 숫자 (가격, 시간, 번호, 점수, 날짜, 수량 등)\n`;
  prompt += `   - 사진에서 본 모든 영어 단어, 문장, 텍스트\n`;
  prompt += `   - 시험지, 문제지, 교재, 학습 자료의 내용\n`;
  prompt += `   - 구체적인 디테일 나열 (예: "책상이 3개", "영어 단어가 보인다")\n`;
  prompt += `   ⚠️ Vision API는 OCR 오인식이 매우 높으므로 숫자/영어는 절대 사용하지 마세요\n`;
  prompt += `🚫 기타 금지:\n`;
  prompt += `   - "소개해볼까해요", "들려드릴게요" 등 부자연스러운 표현\n`;
  prompt += `   - 과도한 홍보성 톤\n`;
  prompt += `   - 제목에 "~후기" 단어\n`;
  prompt += `   - 사진을 너무 디테일하게 묘사하는 것 (후기형일 때 특히 주의)\n\n`;
